...
LIDAR_ANOMALY_THRESHOLD = -0.4     # Less strict threshold for LiDAR
CAMERA_ANOMALY_THRESHOLD = -0.6    # Original camera threshold
LIDAR_CONFIDENCE_THRESHOLD = 0.5   # Adjusted confidence threshold
CAMERA_CONFIDENCE_THRESHOLD = 0.9  # Original camera confidence
DETECTION_WINDOW = 70              # Reduced window for faster detection
TRAINING_PERIOD = 3000            # Increased training samples
MIN_POINTS_THRESHOLD = 100        # Minimum LiDAR points to consider
DETECTION_INTERVAL = 1.0          # Detection check interval in seconds
NOISE_SENSITIVITY = 2.0           # Sensitivity to point cloud variations
...
class SensorSystem:
    def __init__(self):
        ...
        self.previous_lidar_points = None
        self.point_history = deque(maxlen=5)  # Store recent point clouds

    def extract_lidar_features(self, points):
        if len(points) < MIN_POINTS_THRESHOLD:
            return np.zeros(25)  # Increased feature vector size
        
        try:
            # Store point cloud for temporal analysis
            ...
            distance_stats = [
                ...
                np.max(distances),
                np.min(distances),
                np.percentile(distances, 90)  # Added 90th percentile
            ]
            ...
            distances, _ = kdtree.query(points, k=6)  # Get 5 nearest neighbors
            local_density_variations = np.std(distances[:, 1:], axis=1)
            density_features = [
                np.mean(local_density_variations),
                np.std(local_density_variations),
                np.max(local_density_variations)
            ]
            
            # Temporal features
            if len(self.point_history) >= 2:
                prev_points = self.point_history[-2]
                if len(prev_points) > 0:
                    try:
                        ...
                        point_count_change = abs(len(points) - len(prev_points)) / max(len(points), len(prev_points))
                        
                        # Compute density change
                        prev_distances = np.linalg.norm(prev_points, axis=1)
                        density_change = abs(np.mean(distances) - np.mean(prev_distances))
                        
                        temporal_features = [displacement, point_count_change, density_change]
                    except:
                        temporal_features = [0, 0, 0]
                else:
                    temporal_features = [0, 0, 0]
            else:
                temporal_features = [0, 0, 0]
    ...
    def detect_anomalies(self, features, model, threshold, is_lidar=False):
        ...
        try:
            ...
            # Special handling for LiDAR
            if is_lidar:
                # Use rolling statistics for more stable detection
                rolling_mean = np.convolve(scores, np.ones(5)/5, mode='valid')
                rolling_std = np.std(scores[-5:])
                
                # Check for sudden changes in score distribution
                score_gradient = np.gradient(scores[-10:])
                sudden_changes = np.abs(score_gradient) > 0.5
                
                # Combine multiple indicators
                anomaly_indicators = np.array([
                    np.mean(rolling_mean) < LIDAR_ANOMALY_THRESHOLD,
                    rolling_std > 0.3,
                    np.any(sudden_changes)
                ])
                
                confidence = np.mean(anomaly_indicators)
                is_anomaly = confidence > LIDAR_CONFIDENCE_THRESHOLD
                
                return is_anomaly, confidence
            else:
                # Original camera detection logic
                anomaly_scores = scores < threshold
                confidence = np.mean(anomaly_scores)
                is_anomaly = (np.mean(scores) < threshold and 
                            confidence > CAMERA_CONFIDENCE_THRESHOLD)
                return is_anomaly, confidence
    ...
    def check_for_anomalies(self):
        ...
        # Pass is_lidar=True for LiDAR-specific detection
        lidar_anomaly, lidar_confidence = self.detect_anomalies(
            lidar_features, lidar_iforest, LIDAR_ANOMALY_THRESHOLD, is_lidar=True)
        camera_anomaly, camera_confidence = self.detect_anomalies(
            camera_features, camera_iforest, CAMERA_ANOMALY_THRESHOLD, is_lidar=False)
    ...
    try:
        ...
        def process_lidar(point_cloud):
            ...
            if sensor_system.spoof_mode in [1, 3]:
                # Enhanced noise injection for more realistic spoofing
                noise = np.random.normal(0, NOISE_SENSITIVITY, points.shape)
                # Add systematic bias to some points
                mask = np.random.random(len(points)) < 0.3
                noise[mask] *= 2.5  # Increased noise for some points
                # Add occasional outliers
                outlier_mask = np.random.random(len(points)) < 0.05
                noise[outlier_mask] *= 5
                points = points + noise
                
                # Randomly remove some points to simulate sensor blocking
                mask = np.random.random(len(points)) < 0.9
                points = points[mask]
