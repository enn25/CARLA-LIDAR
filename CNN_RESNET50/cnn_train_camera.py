    ...
    def __init__(self, images_path, labels_path, transform=None):
        ...
        for label_file in os.listdir(labels_path):
            with open(os.path.join(labels_path, label_file)) as f:
                ...
                label = 1 if label_data['is_spoofed'] else 0  # 1 for spoofed, 0 for normal
...
dataset = LidarImageDataset(images_path='lidar_dataset/images', labels_path='lidar_dataset/labels', transform=transform)
...
# Define a simple CNN model (Using a pretrained ResNet18 for simplicity)
...
# Train the model and display validation accuracy
train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=10)

# Loading the best model (optional, for later use)
